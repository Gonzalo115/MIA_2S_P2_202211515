...
// agregar genera y devuelve una tabla en formato HTML para el contenido del bloque de carpeta
func (fb *FolderBlock) Agregar() string {
	// Iniciar el contenido de la tabla en formato HTML
	var str = ""

	// Iterar sobre el contenido del bloque de carpeta
	for _, content := range fb.B_content {
		name := string(content.B_name[:])
		// Agregar una fila para cada entrada del bloque
		str += fmt.Sprintf(`
			<tr>
				<td>%s</td>
				<td>%d</td>
			</tr>
		`, name, int(content.B_inodo))
	}
