...
func (inode *Inode) ActualizarI_atime(path string, offset int64) error {
	inode.I_atime = float32(time.Now().Unix())
	err := inode.Serialize(path, offset)
	if err != nil {
		return err
	}
	return nil
...
func (inode *Inode) ActualizarI_mtime(path string, offset int64) error {
